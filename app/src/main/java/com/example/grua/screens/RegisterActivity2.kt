    ...
    private fun setupUI() {
        binding.registerButton.setOnClickListener {
            ...
            when {
                ...
                else -> registerUser(email, password, "operador") // Cambiado a "operador"
            }
        }
        ...
        auth.createUserWithEmailAndPassword(email, password)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    // Crear objeto con datos del operador
                    val userData = hashMapOf(
                        "correo" to email,
                        "tipoUsuario" to userType, // Ahora será "operador"
                        "fechaRegistro" to System.currentTimeMillis()
                        // Puedes agregar más campos específicos para operadores aquí
                    )

                    // Guardar en Firestore
                    db.collection("operario").document(userId)
                        .set(userData)
